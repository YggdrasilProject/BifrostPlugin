...
public class BifrostSSHConnection {
    ...
    protected BifrostSSHConnection(YggdrasilCore serviceCore, String host, int port, BifrostSSHAuth authentication) throws IOException {
        this(serviceCore, host, port, "", authentication);
    }

    protected BifrostSSHConnection(YggdrasilCore serviceCore, String host, int port, String proxy, BifrostSSHAuth authentication) throws IOException {
        core = serviceCore;
        try {
            client = new JSch();
            ...
            core.logInfo("BifrostSSHConnection: Connected: " + connectSession.isConnected());
            connectSession.setServerAliveInterval(ALIVE_INTERVAL);
        } catch (JSchException ex) {
            throw new BifrostSSHException(ex);
        }
    }
    ...
    public synchronized String executeCommand(String command) throws BifrostSSHException {
        if (!isConnected()) {
    ...
    public synchronized void disconnect() {
        if (connectSession != null) {
            core.logInfo("BifrostSSHConnection: Disconnected");
